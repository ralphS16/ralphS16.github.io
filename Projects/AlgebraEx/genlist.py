...
listtext = "<h1>List of exercises from Abstract Algebra by Dummit and Foote (3rd. ed.)</h1>\n"
...
for chdir in sorted(os.listdir("./Exercises")): #look at  chapter directories (have the form 00-19)
	...
	if os.path.isdir(chdirpath):
		if chdir.isdigit():
			...
			listtext += "<h2>Chapter %d</h2>\n<table>\n" %(chapter) if chapter else "<h2>Preliminaries</h2>\n<table>\n" #append the chapter title (handle chapter 0)
